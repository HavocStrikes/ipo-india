...
function sentimentScore(reviews) {
  ...
  return { pts: pos * 15, note: `${reviews.subscribe}/${total} community reviews positive` };
}

/** Colour tone for the score ring/bars — purely visual, never advice. */
...
function computeScore(ipo) {
  ...
  return { score, tone: toneOf(score), confidence, pillars, isSME };
}
