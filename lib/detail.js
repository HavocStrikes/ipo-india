...
/** Chittorgarh URL slug from the company name — fallback for list rows that
 *  lack a slug (upcoming IPOs often miss ~urlrewrite_folder_name). Verified
 *  against chittorgarh: "PhysicsWallah Ltd." -> physicswallah-ipo/2266. */
function slugifyName(name) {
  return String(name || '')
    ...
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .replace(/-(ltd|limited|private|pvt)$/, '');
}

async function loadDetail(ipo) {
  // Prefer the scraped URL; fall back to slug; finally guess from the name so
  // EVERY IPO gets a detail page (charts) even when the list row had no link.
  let url = ipo.detailUrl;
  if (!url && ipo.id) {
    const slug = ipo.slug || slugifyName(ipo.name);
    if (slug) url = `https://www.chittorgarh.com/ipo/${slug}-ipo/${ipo.id}/`;
  }
  if (!url) return null;
  const html = await fetchPage(url, { timeoutMs: 20000, retries: 1 });
  return parseDetail(html);
}

module.exports = { parseDetail, loadDetail, slugifyName, stripTags, mergeDetailIntoIpo, isoFromDisplay };
